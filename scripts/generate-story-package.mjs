...
function packageData(source, prompts, languageRules) {
  return {
    ...
    assets: source.assets.assets.map((asset) => ({
      ...asset,
      url: publicAssetUrl(registry, source.story.slug, asset),
    })),
...
// The app-side map of slug -> storage URL. Used by the marketing pages and the companion avatar
// directly, and by the player only as the fallback when the backend's content response carries
// no asset list (see story-registry.ts).
function renderAppAssets(allSources) {
  ...
  for (const source of allSources) {
    ...
    for (const asset of source.assets.assets) {
      ...
      lines.push(`    ${quote(asset.slug)}: { uri: ${quote(publicAssetUrl(registry, source.story.slug, asset))} },`);
    }
    lines.push('  },');
  }
  lines.push(
