 ...
 * 확정되므로 지금은 "준비 중" 상태 안내와, 이미 동작하는 계정 삭제 흐름으로의 링크만 둔다.
 * 문서 URL이 결정되면 아래 openDoc()의 place-holder를 실제 URL로 교체하면 되도록 한 곳에 모아 뒀다.
 */

// TODO: 정식 URL이 확정되면 여기를 교체하고 openDoc()의 안내 alert를 지운다.
const TERMS_URL = '';
const PRIVACY_URL = '';
const SUPPORT_EMAIL = 'support@qstory.co.kr';

async function openDoc(url: string, fallbackTitle: string) {
  if (!url) {
    if (typeof window !== 'undefined') window.alert?.(`${fallbackTitle}은 곧 공개돼요.`);
    return;
  }
  try {
...
export function MyPagePrivacyPage() {
  ...
  const sections: Section[] = [
    {
      title: '서비스 이용약관',
      body: '서비스를 이용하며 지켜야 할 약속과 회사의 책임 범위를 확인해요.',
      action: { label: '약관 보기', onPress: () => openDoc(TERMS_URL, '이용약관') },
    },
    {
      title: '개인정보 처리방침',
      body: '수집되는 정보와 사용 목적, 보관 기간, 파기 절차를 안내해요.',
      action: { label: '방침 보기', onPress: () => openDoc(PRIVACY_URL, '개인정보 처리방침') },
    },
    {
      title: '내 데이터 열람·내보내기',
